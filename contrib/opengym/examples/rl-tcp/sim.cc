...
main(int argc, char* argv[])
{
    ...
    double duration = 600.0; // 再次延长仿真时间，便于多次突发和RL收敛
    uint32_t run = 0;
    ...
    // 多组突发流量: 多次注入UDP流，制造多次突发拥塞
    for (int burstIdx = 0; burstIdx < 3; ++burstIdx)
    {
        uint16_t burstPort = 60000 + burstIdx;
        OnOffHelper burstApp("ns3::UdpSocketFactory", Address());
        burstApp.SetAttribute("OnTime", StringValue("ns3::ConstantRandomVariable[Constant=1]"));
        burstApp.SetAttribute("OffTime", StringValue("ns3::ConstantRandomVariable[Constant=0]"));
        // 每次突发带宽递增，挑战更大
        std::string burstRate = std::to_string(20 + burstIdx * 10) + "Mbps";
        burstApp.SetAttribute("DataRate", StringValue(burstRate));
        burstApp.SetAttribute("PacketSize", UintegerValue(mtu_bytes));
        AddressValue burstRemote(InetSocketAddress(d.GetRightIpv4Address(0), burstPort));
        burstApp.SetAttribute("Remote", burstRemote);
        ApplicationContainer burstClient = burstApp.Install(d.GetLeft(0));
        // 多次突发，分布在仿真不同阶段
        double burstStart = start_time + duration * (0.2 + 0.25 * burstIdx);
        double burstEnd = burstStart + 20.0 + 10.0 * burstIdx; // 持续时间递增
        burstClient.Start(Seconds(burstStart));
        burstClient.Stop(Seconds(burstEnd));

        // UDP接收端
        PacketSinkHelper burstSink("ns3::UdpSocketFactory",
                                   InetSocketAddress(Ipv4Address::GetAny(), burstPort));
        ApplicationContainer burstSinkApp = burstSink.Install(d.GetRight(0));
        burstSinkApp.Start(Seconds(burstStart - 0.5));
        burstSinkApp.Stop(Seconds(burstEnd + 1.0));
    }
